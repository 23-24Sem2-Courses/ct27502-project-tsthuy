...
if (isset($_GET['keyword'])) {
    ...
    $sql_search = "SELECT * FROM docgia WHERE TenDocGia LIKE ? OR DiaChi LIKE ? OR SoThe LIKE ?";
...
<body>
    ...
    <div class="container mt-5">
        ...
        <div class="row">
            <div class="col-md-6">
                <form class="form-inline mb-3" action="/reader.php" method="get">
            ...
            <div class="col-md-3">
                <select class="form-control" onchange="location = this.value;">
                    <option value="/reader.php?sort_by=TenDocGia" <?php if ($sort_by == 'TenDocGia') echo 'selected'; ?>>Sắp xếp theo Tên</option>
                    <option value="/reader.php?sort_by=DiaChi" <?php if ($sort_by == 'DiaChi') echo 'selected'; ?>>Sắp xếp theo Địa chỉ</option>
                    <option value="/reader.php?sort_by=SoThe" <?php if ($sort_by == 'SoThe') echo 'selected'; ?>>Sắp xếp theo Số thẻ</option>
                </select>
            </div>
            <div class="col-md-3">
        ...
        <div class="mt-4">
            <form id="add-reader-form" style="display: none;" action="/reader.php" method="post">
                <div class="form-group">
                    <label for="TenDocGia">Mã độc giả:</label>
                    <input type="text" class="form-control" id="MaDocGia" name="MaDocGia" required>
                </div>
                <div class="form-group">
                ...
                <tbody>
                    <?php foreach ($readers as $reader) : ?>
                        <tr>
                            ...
                            <td>
                                <a href="/EditReader.php?id=<?= htmlspecialchars($reader['MaDocGia']) ?>" class="btn btn-xs btn-warning">
                                    <i class="fa fa-pencil"></i> Sửa
                                </a>
                                <form action="DeleteReader.php" method="POST" style="display: inline;">
                                    <input type="hidden" name="id" value="<?= htmlspecialchars($reader['MaDocGia']) ?>">
